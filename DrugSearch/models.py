...
class DrugAge(models.Model):
    ...
    def ageNum(**kwargs):
        country_id = kwargs.get('country_id')
        result = DrugAge.objects.raw('SELECT age_num.country_id,age_num.year,age.age,age_num.num FROM age_num,age WHERE age_num.age_id=age.id AND age_num.country_id = %s ORDER BY year',
                                         [country_id])
        return result
...
class DrugGender(models.Model):
    ...
    def genderNum(**kwargs):
        country_id = kwargs.get('country_id')
        result = DrugGender.objects.raw('SELECT gender_num.country_id, gender_num.num, gender_num.year, gender.gender FROM gender_num,gender WHERE gender_num.gender_id = gender.id AND country_id = %s ORDER BY year',
                                         [country_id])
        return result
...
class DrugType(models.Model):
    ...
    def drugTypeNum(**kwargs):
        country_id = kwargs.get('country_id')
        result = DrugAge.objects.raw('SELECT drug_num.country_id,drug_num.year,drug_type.ch_name,drug_num.num FROM drug_num,drug_type WHERE drug_num.drug_id=drug_type.id AND drug_num.country_id = %s ORDER BY year',
                                         [country_id])
        return result
